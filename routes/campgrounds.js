...
router.post("/", middleware.isLoggedIn, function(req, res){
    geocoder.geocode(req.body.campground.location, function(err, data){
        if(err){
            req.flash("failure", "Algo de errado aconteceu!");
            res.redirect("back");
        } else {
            ...
            Campground.create(campground, function(err, campground){
                if(err) {
                    ...
                    req.flash("success", "Acampamento criado!");
...
router.get("/:id", function(req, res){
    ...
    Campground.findById(id).populate("comments").exec(function(err, campground){
        if(err || !campground) {
            req.flash("failure", "Acampamento não encontrado!");
            res.redirect("back");
        } else {
        ...
        if(err){
            ...
            Campground.findByIdAndUpdate(id, updatedCampground, function(err, campground){
                if(err || !campground) {
                    ...
                    req.flash("success", "Acampamento atualizado!");
...
router.delete("/:id", middleware.checkCampgroundOwner, function(req, res){
    ...
    Campground.findByIdAndRemove(id, function(err){
        if(err){
            ...
            req.flash("success", "Acampamento deletado!");
            res.redirect("/campgrounds");
        }
    });
});
